(() => {
	function ask(data, cb) {
		const publicURL = 'https://guideme.penphy.com'
		$.post({ url: publicURL + '/lib/webchat.php', data: data }).done(cb);
	}
	...
	const roomEvents = [
		...
		'NewReadCursor' //		(roomId, cursor) // User's cursor changed
	]
	...
	class WebChat {
		constructor(userID) {
			...
			new Chatkit.ChatManager({
				instanceLocator: 'v1:us1:c2c8e696-a9e8-407b-934b-5baa4fb0f4d0',
				userId: userID,
				tokenProvider: new Chatkit.TokenProvider({url: 'https://us1.pusherplatform.io/services/chatkit_token_provider/v1/c2c8e696-a9e8-407b-934b-5baa4fb0f4d0/token'})
			}).connect(MyEvent.createEventCallback(this.events.user)).then(user => {
				this.user = user;
				this.getAllRooms(room => this.subscribe(room.id));
				this.on('AddedToRoom', room => this.subscribe(room.id));
				this.ready.forEach(e => e());
				this.loaded = 1;
				this.readyLastRun();
			});
		}
		...
		createRoom(data, cb = emptyFn) {
			if (typeof(data.name) == 'undefined') data.name = this.user.id + ' ' + (data.addUserIds ? data.addUserIds[0] : '');
			data.private = true;
		...
		/*
			...
			event return Array[] from older to newer messages
		...
		*/
		...
		setReadCursor(roomId, msgId, cb = emptyFn) {
			let pos = this.getReadCursor(roomId)
			if (!pos || pos.position != msgId) {
				this.user.setReadCursor({roomId: roomId, position: msgId}).then(cb);
			}
		}
	
		// return Cursor object
		getReadCursor(roomId, userId) {
			return this.user.readCursor({roomId: roomId, userId: userId});
		}
	
		// subscribe to a room to active roomEvents
		subscribe(roomId, messageLimited) {
			return this.user.subscribeToRoomMultipart({
				roomId: roomId,
				messageLimit: messageLimited || 0,
...
/**
	...
	@name Cursor
		@property position (int): The message ID that the user has read up to.
		@property updatedAt (User): The user who sent the message.
		@property room (Room): The timestamp when the cursor was last set.
		@property user (User): The The user that the cursor belongs to.
		@property type (int): Currently always 0 (representing a read cursor).
*/