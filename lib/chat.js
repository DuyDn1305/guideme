const chatServer = '/webchat/lib/chat.php';
...
class MyEvent {
	...
	static createEventCallback(eventHandler) {
		let events = {}
		for (let key in eventHandler) {
			events[`on${key}`] = function(...params) {
				eventHandler[key].trigger(...params);
			}
		}
		return events;
	}
...
class WebChat {
	...
	createRoom(data, cb = () => {}) {
		if (empty(data.name)) data.name = `Room of ${this.user.id}`;
		if (empty(data.addUserIds)) data.addUserIds = [];
		data.private = true;
 ...
 */
	subscribe(roomId, messageLimit) {
		this.user.subscribeToRoomMultipart({
			...
			hooks: MyEvent.createEventCallback(this.events.room)
		});
	}
}
